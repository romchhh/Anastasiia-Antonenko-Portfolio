    ...
    <div className="min-h-screen bg-[#F5F5F5] relative overflow-x-hidden">
      ...
      <header className="md:hidden bg-transparent relative">
        <div className="px-5 pt-4 pb-4 flex items-center justify-between">
          <h1 className="text-[22px] font-normal leading-[110%] tracking-[-0.01em] text-[#1A1A1A] uppercase font-sans">FROM HEADLINES TO REALITY</h1>
      ...
      <div className="md:mx-3 lg:mx-6 md:my-3 lg:my-6 bg-transparent relative pr-0 md:pr-[17rem]">
        <div className="hidden md:block"><Header title="FROM HEADLINES TO REALITY" subtitle="anastasiia antonenko" /></div>
        <main className="py-2 md:py-4 lg:py-8 px-5 md:px-2 lg:pl-12 lg:pr-8">
              ...
              <div className="mb-6 text-[#1A1A1A] relative">
                ...
                <div className="grid grid-cols-2 gap-4 md:gap-6 relative">
                  ...
                  <div className="pr-4">
                    ...
                    <p className="mt-1 text-[16px] font-normal leading-[150%] tracking-[-0.01em] lowercase" style={{ fontFamily: 'Work Sans' }}>2023–2024</p>
                  </div>
                  
                  {/* Medium column */}
                  <div className="pl-4">
                    <p className="text-[16px] font-medium leading-[150%] tracking-[-0.01em] lowercase" style={{ fontFamily: 'Work Sans' }}>medium:</p>
                    <p className="mt-1 text-[16px] font-normal leading-[150%] tracking-[-0.01em] lowercase" style={{ fontFamily: 'Work Sans' }}>collage of newspaper prints and analog photography on heavy paper, tea-dyed</p>
                  </div>
                </div>
            ...
            <div className="hidden lg:block w-full origin-top-left transition-transform duration-200 ease-out -ml-12 -mr-8 mt-6" style={{ transform: `scale(${scale})`, height: `${2738 * scale}px` }}>
              <div className="relative" style={{ width: '1138px', height: '2738px' }}>
                {galleryImages.map((image, index) => (
                  <div key={index} className="absolute overflow-hidden bg-white/0 cursor-pointer hover:opacity-80 transition-opacity" style={{ width: `${image.width}px`, height: `${image.height}px`, top: `${image.top}px`, left: `${image.left}px` }} onClick={() => openModal(index)}>
                    <img src={image.src} alt={`From Headlines to Reality ${index + 1}`} className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
            </div>
            <div className="hidden md:grid lg:hidden grid-cols-2 gap-4">
              {galleryImages.map((image, index) => (
                <div key={index} className="cursor-pointer hover:opacity-80 transition-opacity" onClick={() => openModal(index)}>
                  <div className="overflow-hidden bg-white/0"><img src={image.src} alt={`From Headlines to Reality ${index + 1}`} className="w-full h-auto object-cover" /></div>
                </div>
              ))}
            </div>
