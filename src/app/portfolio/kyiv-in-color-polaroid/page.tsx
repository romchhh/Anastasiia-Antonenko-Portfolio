    ...
    <div className="min-h-screen bg-[#F5F5F5] relative overflow-x-hidden">
      ...
      <header className="md:hidden bg-transparent relative">
        <div className="px-5 pt-4 pb-4 flex items-center justify-between">
          <h1 className="text-[22px] font-normal leading-[110%] tracking-[-0.01em] text-[#1A1A1A] uppercase font-sans">KYIV IN COLOR. POLAROID EDITION</h1>
      ...
      <div className="md:mx-3 lg:mx-6 md:my-3 lg:my-6 bg-transparent relative pr-0 md:pr-[17rem]">
        <div className="hidden md:block"><Header title="KYIV IN COLOR. POLAROID EDITION" subtitle="anastasiia antonenko" /></div>
        <main className="py-2 md:py-4 lg:py-8 px-5 md:px-2 lg:pl-12 lg:pr-8">
            <div className="hidden lg:grid grid-cols-[260px_1px_1fr] gap-6 items-start">
              <aside className="text-[#1A1A1A]">
                <div className="space-y-8 pr-6 -ml-6">
                  <div><p className="text-[22px] font-medium leading-[150%] tracking-[0.03em] lowercase">years:</p><p className="mt-2 text-[16px] font-normal leading-[150%] tracking-[0.03em]">2022–2023</p></div>
                  <div><p className="text-[22px] font-medium leading-[150%] tracking-[0.03em] lowercase">medium:</p><p className="mt-2 text-[16px] font-normal leading-[150%] tracking-[0.03em]">Polaroid photography</p></div>
                </div>
              </aside>
              <div className="bg-gray-300 w-px self-stretch -mt-8" />
              <section>
                <p className="text-[18px] font-normal leading-[150%] tracking-[0.03em] text-[#1A1A1A] max-w-[680px] whitespace-pre-line">{fullStoryText}</p>
                <div className="relative mt-14"><div className="absolute top-0 h-px bg-gray-300" style={{ left: 'calc(-2rem - 260px - 1.5rem - 1px - 1.5rem)', right: 'calc(-2rem - 3%)' }} /></div>
              </section>
            </div>
            <div className="lg:hidden mb-6">
              <p className="text-[14px] md:text-[16px] font-normal leading-[150%] tracking-[0.03em] text-[#1A1A1A] whitespace-pre-line pr-4 mb-4">{fullStoryText}</p>
              ...
              <div className="mb-6 text-[#1A1A1A] relative">
                ...
                <div className="grid grid-cols-2 gap-4 md:gap-6 relative">
                  ...
                  {/* Years column */}
                  <div className="pr-4">
                    <p className="text-[16px] font-medium leading-[150%] tracking-[-0.01em] lowercase" style={{ fontFamily: 'Work Sans' }}>years:</p>
                    <p className="mt-1 text-[16px] font-normal leading-[150%] tracking-[-0.01em] lowercase" style={{ fontFamily: 'Work Sans' }}>2022–2023</p>
                  </div>
                  
                  {/* Medium column */}
                  <div className="pl-4">
                    <p className="text-[16px] font-medium leading-[150%] tracking-[-0.01em] lowercase" style={{ fontFamily: 'Work Sans' }}>medium:</p>
                    <p className="mt-1 text-[16px] font-normal leading-[150%] tracking-[-0.01em] lowercase" style={{ fontFamily: 'Work Sans' }}>Polaroid photography</p>
                  </div>
                </div>
            ...
            <div className="hidden lg:block w-full origin-top-left transition-transform duration-200 ease-out -ml-12 -mr-8 mt-6" style={{ transform: `scale(${scale})`, height: `${4724 * scale}px` }}>
              <div className="relative" style={{ width: '1136px', height: '4724px' }}>
                {galleryImages.map((image, index) => (
                  <div key={index} className="absolute overflow-hidden bg-white/0 cursor-pointer hover:opacity-80 transition-opacity" style={{ width: `${image.width}px`, height: `${image.height}px`, top: `${image.top}px`, left: `${image.left}px` }} onClick={() => openModal(index)}>
                    <img src={image.src} alt={`Kyiv in Color Polaroid ${index + 1}`} className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
            </div>
            <div className="hidden md:grid lg:hidden grid-cols-2 gap-4">
              {galleryImages.map((image, index) => (
                <div key={index} className="cursor-pointer hover:opacity-80 transition-opacity" onClick={() => openModal(index)}>
                  <div className="overflow-hidden bg-white/0"><img src={image.src} alt={`Kyiv in Color Polaroid ${index + 1}`} className="w-full h-auto object-cover" /></div>
                </div>
              ))}
            </div>
      ...
      {selectedImage !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={closeModal}>
          ...
          <div className="relative mx-4 flex items-center justify-center">
            <img src={galleryImages[selectedImage].src} alt={`Kyiv in Color Polaroid ${selectedImage + 1}`} className="h-[85vh] w-auto max-w-[90vw] object-contain" onClick={(e) => e.stopPropagation()} />
